	...
	public class Slots : IGame
	{
		...
		public void Play(Player player)
		{
			...
			while (true)
			{
				...
				Console.WriteLine();
				Console.Write("Enter bet (or 0 to return): ");
				...
				{
					Console.WriteLine("Invalid bet or not enough chips. Press any key to continue...");
				...
				ShowResult(bet, player, jackpot, win);

				if (player.Chips <= 0)
				{
					Console.WriteLine("You're out of chips. Press any key to return to lobby...");
					Console.ReadKey(true);
					break;
				}

				Console.WriteLine("Press Space to play again, Esc to return to lobby.");
				var key = Console.ReadKey(true).Key;
				if (key == ConsoleKey.Escape) break;
				// any other key - continue (space recommended)
			}
		...
		private int CenterX(string text) => Math.Max(0, (Console.WindowWidth - text.Length) / 2);

		private void WriteCentered(string text, int y)
		{
			try { Console.SetCursorPosition(CenterX(text), y); } catch { }
			Console.Write(text);
		}
		...
		private void WriteCenteredBlock(string block, int startY)
		{
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				try { Console.SetCursorPosition(CenterX(line), startY + i); } catch { }
				Console.Write(line);
			}
		}
...
";
		...
		private void DrawMenu()
		{
			...
			WriteCentered(" PRESS SPACE TO SPIN (enter bet then press Enter) ", 8);

			Console.ForegroundColor = ConsoleColor.DarkGray;
			WriteCentered("(Press 0 to return to lobby)", 9);
...
";
		...
		private void ShowResult(int bet, Player player, bool jackpot, bool win)
		{
			int y = 20;
			...
			if (jackpot)
			{
				...
				y++;
				Console.ForegroundColor = ConsoleColor.Green;
				int jackpotPayout = bet * 10;
                WriteCentered($"{jackpotPayout} CREDITS AWARDED!", y);
			...
			else if (win)
			{
				...
				int payout = bet * 2;
				Console.ForegroundColor = ConsoleColor.Green;
				WriteCentered($"{payout} CREDITS AWARDED!", y);
